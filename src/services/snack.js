...
export const getHealthySnack = async () => {
  const collectionRef = collection(db, "snack");

  const healthySnackQuery = query(
    collectionRef,
    where("category", "==", "healthy")
  );

  const snapshot = await getDocs(healthySnackQuery);
...
export const getGiftPackSnack = async () => {
  const collectionRef = collection(db, "snack");
...
export const cleanSnackData = (SnackData) => {
  const defaultLnk =
    "https://cloudinary.images-iherb.com/image/upload/f_auto,q_auto:eco/images/snl/snl68752/l/8.jpg";
